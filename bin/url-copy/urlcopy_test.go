...
// copy function.
func Serialize(t *testing.T, task *tasks.Batch) (file string) {
...
func ConsumeStartMessages(t *testing.T) []*tasks.Transfer {
	startList := make([]*tasks.Transfer, 0, 10)
...
func ConsumeEndMessages(t *testing.T) []*tasks.Transfer {
	endList := make([]*tasks.Transfer, 0, 10)
...
func TestSimpleTransfer(t *testing.T) {
	...
	task := &tasks.Batch{
		...
		Transfers: []*tasks.Transfer{transfer},
	}
	...
	if end[0].Status.Error != nil {
		t.Errorf("Expecting success, got %s", end[0].Status.Error.Description)
	}
	if end[0].Status.Stats.TransferredBytes != 10 {
		t.Errorf("Expecting filesize to be reported 10, got %d", end[0].Status.Stats.TransferredBytes)
	}
}
...
func TestPanic(t *testing.T) {
	...
	} else if end[0].Status.Error.Code != syscall.EINTR {
		t.Errorf("Expecting EINTR, got %d", end[0].Status.Error.Code)
	} else if end[0].Status.Error.Description != "TEST PANIC MESSAGE" {
		t.Errorf("Unexpected error message, got %s", end[0].Status.Error.Description)
	}
}
...
func TestCancel(t *testing.T) {
	...
	if end[0].Status.Error == nil {
		...
		return
	} else if end[0].Status.Error.Code != syscall.ECANCELED {
		t.Errorf("Expecting ECANCELED, got %d", end[0].Status.Error.Code)
	}
}
...
func TestTimeout(t *testing.T) {
	...
	transfer := &tasks.Transfer{
		...
		Parameters: tasks.TransferParameters{
			Timeout: timeout,
		},
	}
	...
	if end[0].Status.Error == nil {
		t.Errorf("Expecting an error, got success")
	} else if end[0].Status.Error.Code != syscall.ETIMEDOUT {
		t.Errorf("Expecting ETIMEDOUT, got %d", end[0].Status.Error.Code)
	}
}
...
func TestMultipleSimple(t *testing.T) {
	...
	if end[0].Status.Error != nil {
		t.Errorf("First transfer expected success, got %s", end[0].Status.Error.Description)
		return
	}
	if end[1].Status.Error != nil {
		t.Errorf("Second transfer expected success, got %s", end[1].Status.Error.Description)
		return
	}

	if end[0].Status.Stats.TransferredBytes != 10 {
		t.Errorf("File size for transfer 1 does not match. Got %d", end[0].Status.Stats.TransferredBytes)
	}
	if end[1].Status.Stats.TransferredBytes != 42 {
		t.Errorf("File size for transfer 2 does not match. Got %d", end[1].Status.Stats.TransferredBytes)
	}
}
...
func TestMultipleCancel(t *testing.T) {
	...
	if end[0].Status.Error != nil {
		t.Errorf("First transfer expecting success, got %s", end[0].Status.Error.Description)
	}
	if end[1].Status.Error == nil {
		t.Errorf("Second transfer expecting failure, got success")
	} else if end[1].Status.Error.Code != syscall.ECANCELED {
		t.Errorf("Second transfer expecting ECANCELED, got %d", end[1].Status.Error.Code)
	}
}
...
func TestMultiplePanic(t *testing.T) {
	...
	if end[1].Status.Error == nil {
		t.Errorf("Second transfer expecting failure, got success")
	} else if end[1].Status.Error.Code != syscall.EINTR {
		t.Errorf("Second transfer expecting EINTR, got %d", end[1].Status.Error.Code)
	}
}
...
func TestMultiHop(t *testing.T) {
	...
	} else if end[0].Status.Error.Code != syscall.ENOENT {
		t.Errorf("First transfer expecting ENOENT, got %d instead", end[0].Status.Error.Code)
	}
	if end[1].Status.Error == nil {
		t.Errorf("Second transfer expecting failure, got success")
	} else if end[1].Status.Error.Code != syscall.ECANCELED {
		t.Errorf("Second transfer expecting ECANCELED, got %d instead", end[0].Status.Error.Code)
	}
}
